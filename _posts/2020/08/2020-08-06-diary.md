...
* $5$ 個出力して標本とし、標本平均 $\overline X$、標本分散 $S^2$ を計算することを繰り返す。
  $\overline X$ の期待値、分散はいくらか。また $S^2$ の期待値はいくらか。
...
\begin{aligned}
&E[\overline X] = \mu = 50,\\
&E[(\overline X - \mu)^2] = \dfrac{\sigma^2}{n} \approx 163.333,\\
...
$$
\begin{aligned}
\mu &= E[\overline X] = 62.8,\\
\sigma^2 &= nE[(\overline X - \mu)^2] = 20 \cdot 5.2^2 = 540.8 \approx 23.255^2.
\quad\blacksquare
\end{aligned}
$$
...
解：母集団が $N(100.2, 1.6^2)$ に従うので、標本平均 $\overline X$ は
$N(100.2, 1.6^2/5)$ に従う。
...
$$
\begin{aligned}
Z = \frac{\overline X - \mu}{S/\sqrt{n - 1}}
...
解：$Z = (\overline X - \mu)/(\sigma/\sqrt{n})$ が $N(0, 1)$ に従う。

$$
\begin{aligned}
\overline X &= \mu + \frac{\sigma}{\sqrt{n}}Z \ge 1.5\\
Z &\ge \frac{\sqrt{n}}{\sigma}(1.5 - \mu).\\
\therefore P(\overline X\!\ge\!1.5) &= P\!\left(Z\!\ge\!\frac{\sqrt{n}}{\sigma}(1.5 - \mu)\right)\\
&= P(Z \ge 2).
\end{aligned}
$$
...
このような結果が生じる確率を $5$ パーセントと比較しろ。

解：$F$ 分布を応用する。

$$
\begin{aligned}
X &= \frac{(n - 1)(\overline X - \mu)^2}{S^2}\\
...
$3.9375 \lt 5.5914...$ だから $1$ パーセントより大きいと言える。
$\blacksquare$
